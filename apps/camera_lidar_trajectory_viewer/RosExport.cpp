...
namespace
{
    ...
    sensor_msgs::msg::PointCloud2 makeCloud(const std::string& frame, int64_t stampNs, const std::vector<float>& xyzi)
    {
...
bool exportRos2Bag(const RosExportInput& in, const RosExportOptions& opt, std::string& status)
{
    ...
    if (wantRaw && !haveTraj)
        wantRaw = false; // need poses to undo motion
    ...
    int64_t startTs = 0;
    if (haveTraj)
        startTs = in.traj.poses.front().ts_ns;
    else if (!in.imageFiles.empty())
        startTs = in.imageFiles.begin()->first;
    ...
    try
    {
        ...
        if (opt.exportTf && in.calibLoaded)
        {
            ...
            T_lc.linear() = omFiKaToMat3(in.E.om, in.E.fi, in.E.ka);
        ...
        if (opt.exportCamera && !in.imageFiles.empty())
        {
            ...
            for (const auto& [ts, path] : in.imageFiles)
            {
                ...
                {
                    if (camW == 0)
                    { // copy-bytes path: peek dimensions once
        ...
        if ((opt.exportLidarUndistorted || wantRaw) && !in.lidarChunks.empty())
        {
            ...
            if (!pts.empty())
            {
                ...
                std::fprintf(
                    stderr,
                    "[RosExport] lidar: %zu points, span %.2f s, window %.3f s\n",
                    pts.size(),
                    (pts.back().ts - t0) / 1e9,
                    opt.aggregationSec);
                ...
                while (i < pts.size())
                {
                    ...
                    size_t j = i;
                    while (j < pts.size() && (pts[j].ts - t0) / aggNs == w)
                        ++j;
                    ...
                    if (opt.exportLidarUndistorted)
                    {
                        ...
                        {
                            buf.push_back(pts[k].x);
                            buf.push_back(pts[k].y);
                            buf.push_back(pts[k].z);
                            buf.push_back(pts[k].intensity);
                        }
                        writer.write(makeCloud(in.mapFrame, winStamp, buf), kTopicLidarUndist, rclcpp::Time(winStamp));
                        ++nCloud;
                    }
                    if (wantRaw)
                    {
                        ...
                        writer.write(makeCloud(in.lidarFrame, winStamp, buf), kTopicLidarRaw, rclcpp::Time(winStamp));
                        ++nCloud;
                    }
                    i = j;
                }
    ...
    status = "Wrote bag '" + opt.outUri + "' (" + opt.storageId + "): " + std::to_string(nTf) + " tf, " + std::to_string(nImg) + " img, " +
        std::to_string(nCloud) + " clouds" + (opt.exportLidarRaw && !haveTraj ? "  [raw skipped: no trajectory]" : "");
    return true;
}
