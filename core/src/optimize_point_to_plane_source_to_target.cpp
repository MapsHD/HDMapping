...
Eigen::Matrix<double, 1, 1> get_delta_point_to_plane_tait_bryan(RegistrationPlaneFeature::PoseConvention pose_convention,
...
Eigen::Matrix<double, 1, 6> get_point_to_plane_jacobian_tait_bryan(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
...
Eigen::Matrix<double, 1, 1> get_delta_point_to_plane_rodrigues(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
    Eigen::Matrix<double, 1, 1> delta;
...
Eigen::Matrix<double, 1, 6> get_point_to_plane_jacobian_rodrigues(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
...
Eigen::Matrix<double, 1, 1> get_delta_point_to_plane_quaternion(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
    Eigen::Matrix<double, 1, 1> delta;
...
Eigen::Matrix<double, 1, 7> get_point_to_plane_jacobian_quaternion(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
...
bool RegistrationPlaneFeature::optimize_point_to_plane_source_to_target(PointClouds& point_clouds_container)
{
    ...
    //if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
    //    covariance_matrices_before6x6 = compute_covariance_matrices_tait_bryan_point_to_plane_source_to_target(point_clouds_container, pose_convention);
    //}
    //if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
    //    covariance_matrices_before6x6 = compute_covariance_matrices_rodrigues_point_to_plane_source_to_target(point_clouds_container, pose_convention);
    //}
    //if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
    //    covariance_matrices_before7x7 = compute_covariance_matrices_quaternion_point_to_plane_source_to_target(point_clouds_container, pose_convention);
    //}

    //----------------------------------------------------------------------------------------------------
    optimize_point_to_plane_source_to_target(point_clouds_container, pose_convention, optimization_algorithm, rotation_matrix_parametrization);
    //----------------------------------------------------------------------------------------------------

    //if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
    //    covariance_matrices_after6x6 = compute_covariance_matrices_tait_bryan_point_to_plane_source_to_target(point_clouds_container, pose_convention);
    //}
    //if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
    //    covariance_matrices_after6x6 = compute_covariance_matrices_rodrigues_point_to_plane_source_to_target(point_clouds_container, pose_convention);
    //}
    //if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
    //    covariance_matrices_after7x7 = compute_covariance_matrices_quaternion_point_to_plane_source_to_target(point_clouds_container, pose_convention);
    //}
...
bool RegistrationPlaneFeature::optimize_point_to_plane_source_to_target(PointClouds& point_clouds_container,
    RegistrationPlaneFeature::PoseConvention pose_convention, RegistrationPlaneFeature::OptimizationAlgorithm optimization_algorithm, RegistrationPlaneFeature::RotationMatrixParametrization rotation_matrix_parametrization)
{
            ...
            for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
                if (i != j) {
                    ...
                    for (size_t k = 0; k < nns.size(); k++) {
                        ...
                        Eigen::Matrix<double, 1, 1> delta;
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
                            delta = get_delta_point_to_plane_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
                            delta = get_delta_point_to_plane_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
                            delta = get_delta_point_to_plane_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        ...
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
                            jacobian1x6 = get_point_to_plane_jacobian_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
                            jacobian1x6 = get_point_to_plane_jacobian_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
                            jacobian1x7 = get_point_to_plane_jacobian_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
...
std::vector<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>> RegistrationPlaneFeature::compute_covariance_matrices_tait_bryan_point_to_plane_source_to_target(
        ...
        for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
            if (i != j) {
                ...
                for (size_t k = 0; k < nns.size(); k++) {
                    ...
                    Eigen::Matrix<double, 1, 1> delta = get_delta_point_to_plane_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);

                    if (!(delta(0, 0) == delta(0, 0))) {
                        continue;
                    }
                    
                    Eigen::Matrix<double, 1, 6> jacobian = get_point_to_plane_jacobian_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
...
std::vector<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>> RegistrationPlaneFeature::compute_covariance_matrices_rodrigues_point_to_plane_source_to_target(
    PointClouds& point_clouds_container, PoseConvention pose_convention)
{
    for (auto& pc : point_clouds_container.point_clouds) {
        pc.build_rgd();
        ...
        for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
            if (i != j) {
                ...
                for (size_t k = 0; k < nns.size(); k++) {
                    ...
                    Eigen::Matrix<double, 1, 1> delta = get_delta_point_to_plane_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);

                    if (!(delta(0, 0) == delta(0, 0))) {
                        continue;
                    }
                    
                    Eigen::Matrix<double, 1, 6> jacobian = get_point_to_plane_jacobian_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
...
std::vector<Eigen::Matrix<double, 7, 7, Eigen::RowMajor>> RegistrationPlaneFeature::compute_covariance_matrices_quaternion_point_to_plane_source_to_target(
    PointClouds& point_clouds_container, PoseConvention pose_convention)
{
    for (auto& pc : point_clouds_container.point_clouds) {
        pc.build_rgd();
        ...
        for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
            if (i != j) {
                ...
                for (size_t k = 0; k < nns.size(); k++) {
                    ...
                    Eigen::Matrix<double, 1, 1> delta = get_delta_point_to_plane_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);

                    if (!(delta(0, 0) == delta(0, 0))) {
                        continue;
                    }
                    
                    Eigen::Matrix<double, 1, 7> jacobian = get_point_to_plane_jacobian_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
