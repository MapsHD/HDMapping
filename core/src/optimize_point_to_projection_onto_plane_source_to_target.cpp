...
Eigen::Matrix<double, 3, 1> get_delta_point_to_projection_onto_plane_tait_bryan(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
    ...
    if (pose_convention == RegistrationPlaneFeature::PoseConvention::wc) {
        point_to_projection_onto_plane_tait_bryan_wc(
            ...
            p_s.x(), p_s.y(), p_s.z(),
            p_t.x(), p_t.y(), p_t.z(), nv_t.x(), nv_t.y(), nv_t.z());
    }
    if (pose_convention == RegistrationPlaneFeature::PoseConvention::cw) {
...
Eigen::Matrix<double, 3, 1> get_delta_point_to_projection_onto_plane_rodrigues(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
    Eigen::Matrix<double, 3, 1> delta;
...
Eigen::Matrix<double, 3, 1> get_delta_point_to_projection_onto_plane_quaternion(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
    Eigen::Matrix<double, 3, 1> delta;
...
Eigen::Matrix<double, 3, 6> get_point_to_projection_onto_plane_jacobian_tait_bryan(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
...
Eigen::Matrix<double, 3, 6> get_point_to_projection_onto_plane_jacobian_rodrigues(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
...
Eigen::Matrix<double, 3, 7> get_point_to_projection_onto_plane_jacobian_quaternion(RegistrationPlaneFeature::PoseConvention pose_convention,
    Eigen::Affine3d m_pose_wc, Eigen::Vector3d p_s, Eigen::Vector3d p_t, Eigen::Vector3d nv_t) {
...
bool RegistrationPlaneFeature::optimize_point_to_projection_onto_plane_source_to_target(PointClouds& point_clouds_container) {
    ...
    if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
        covariance_matrices_before6x6 = compute_covariance_matrices_tait_bryan_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention);
    }
    if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
        covariance_matrices_before6x6 = compute_covariance_matrices_rodrigues_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention);
    }
    if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
        covariance_matrices_before7x7 = compute_covariance_matrices_quaternion_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention);
    }*/
     
    //----------------------------------------------------------------------------------------------------
    optimize_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention, optimization_algorithm, rotation_matrix_parametrization);
    //----------------------------------------------------------------------------------------------------
        
    /*if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
        covariance_matrices_after6x6 = compute_covariance_matrices_tait_bryan_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention);
    }
    if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
        covariance_matrices_after6x6 = compute_covariance_matrices_rodrigues_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention);
    }
    if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
        covariance_matrices_after7x7 = compute_covariance_matrices_quaternion_point_to_projection_onto_plane_source_to_target(point_clouds_container, pose_convention);
    }
...
void alpha_point_to_projection_job(RegistrationPlaneFeature::Job* job, std::vector<double>* alphas, float barron_c, std::vector<std::vector<std::pair<int, int>>>* all_nns,
    ...
    std::vector<double>* sums_x, std::vector<double>* sums_y, std::vector<double>* sums_z, int index_source, RegistrationPlaneFeature::PoseConvention pose_convention, 
        ...
        for (int i = 0; i < point_clouds_container.point_clouds.size(); i++) {
            ...
            if (is_adaptive_robust_kernel) {
                ...
                for (size_t k = 0; k < jobs.size(); k++) {
                    threads.push_back(std::thread(alpha_point_to_projection_job, &jobs[k], &alphas, barron_c, &all_nns, &point_clouds_container.point_clouds, &j_indexes,
            ...
            for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
                if (i != j) {
                    ...
                    for (size_t k = 0; k < nns.size(); k++) {
                        ...
                        Eigen::Matrix<double, 3, 1> delta;
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
                            delta = get_delta_point_to_projection_onto_plane_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
                            delta = get_delta_point_to_projection_onto_plane_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
                            delta = get_delta_point_to_projection_onto_plane_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        ...
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::tait_bryan_xyz) {
                            jacobian3x6 = get_point_to_projection_onto_plane_jacobian_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::rodrigues) {
                            jacobian3x6 = get_point_to_projection_onto_plane_jacobian_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
                        if (rotation_matrix_parametrization == RotationMatrixParametrization::quaternion) {
                            jacobian3x7 = get_point_to_projection_onto_plane_jacobian_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                        }
...
//-
bool RegistrationPlaneFeature::optimize_point_to_projection_onto_plane_source_to_target_lie_algebra_left_jacobian(PointClouds& point_clouds_container)
{
            ...
            for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
                if (i != j) {
                    ...
                    for (size_t k = 0; k < nns.size(); k++) {
                        ...
                        Eigen::Vector3d p_tg(point_clouds_container.point_clouds[j].m_pose * point_clouds_container.point_clouds[j].points_local[nns[k].second]);
                        Eigen::Vector3d nv_t(point_clouds_container.point_clouds[j].m_pose.rotation() * point_clouds_container.point_clouds[j].normal_vectors_local[nns[k].second]);
                        if (nv_t.norm() != 1.0) {
                            continue;
                        }
...
bool RegistrationPlaneFeature::optimize_point_to_projection_onto_plane_source_to_target_lie_algebra_right_jacobian(PointClouds& point_clouds_container)
{
    int number_of_unknowns_per_pose = 6;
...
//-
std::vector<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>> RegistrationPlaneFeature::compute_covariance_matrices_tait_bryan_point_to_projection_onto_plane_source_to_target(
        ...
        for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
            if (i != j) {
                ...
                for (size_t k = 0; k < nns.size(); k++) {
                    ...
                    Eigen::Matrix<double, 3, 1> delta = get_delta_point_to_projection_onto_plane_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                    ...
                    Eigen::Matrix<double, 3, 6> jacobian = get_point_to_projection_onto_plane_jacobian_tait_bryan(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
...
std::vector<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>> RegistrationPlaneFeature::compute_covariance_matrices_rodrigues_point_to_projection_onto_plane_source_to_target(
    PointClouds& point_clouds_container, PoseConvention pose_convention)
{
    for (auto& pc : point_clouds_container.point_clouds) {
        pc.build_rgd();
        ...
        for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
            if (i != j) {
                ...
                for (size_t k = 0; k < nns.size(); k++) {
                    ...
                    Eigen::Matrix<double, 3, 1> delta = get_delta_point_to_projection_onto_plane_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                    ...
                    Eigen::Matrix<double, 3, 6> jacobian = get_point_to_projection_onto_plane_jacobian_rodrigues(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
...
std::vector<Eigen::Matrix<double, 7, 7, Eigen::RowMajor>> RegistrationPlaneFeature::compute_covariance_matrices_quaternion_point_to_projection_onto_plane_source_to_target(
    PointClouds& point_clouds_container, PoseConvention pose_convention)
{
    for (auto& pc : point_clouds_container.point_clouds) {
        pc.build_rgd();
        ...
        for (int j = 0; j < point_clouds_container.point_clouds.size(); j++) {
            if (i != j) {
                ...
                for (size_t k = 0; k < nns.size(); k++) {
                    ...
                    Eigen::Matrix<double, 3, 1> delta = get_delta_point_to_projection_onto_plane_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
                    ...
                    Eigen::Matrix<double, 3, 7> jacobian = get_point_to_projection_onto_plane_jacobian_quaternion(pose_convention, point_clouds_container.point_clouds[i].m_pose, p_s, p_t, nv_t);
