...
bool PointCloud::load_pc(std::string input_file_name, bool load_cache_mode)
{
	...
	if(!load_cache_mode){
		this->m_pose = Eigen::Affine3d::Identity();
		this->m_initial_pose = this->m_pose;
		this->pose = pose_tait_bryan_from_affine_matrix(this->m_pose);
		this->gui_translation[0] = static_cast<float>(this->pose.px);
		this->gui_translation[1] = static_cast<float>(this->pose.py);
		this->gui_translation[2] = static_cast<float>(this->pose.pz);
		this->gui_rotation[0] = rad2deg(this->pose.om);
...
inline void gpuldu3(double *A, int *P)
{
	...
	P[0] = abs(A[3 * 1 + 0]) > abs(A[3 * 0 + 0]) ? (abs(A[3 * 2 + 0]) > abs(A[3 * 1 + 0]) ? 2 : 1) : (abs(A[3 * 2 + 0]) > abs(A[3 * 0 + 0]) ? 2 : 0);
	P[P[0]] = 0;
...
inline void gpuSVD(double *_A, double *U, double *_S, double *V)
{
	...
	y[0] = y[1] = y[2] = 0;
	k = abs(LDU[P[1]][1]) < abs(LDU[P[0]][0]) ? (abs(LDU[P[2]][2]) < abs(LDU[P[1]][1]) ? 2 : 1) : (abs(LDU[P[2]][2]) < abs(LDU[P[0]][0]) ? 2 : 0);
	y[k] = 1;
	...
	/* Pick the component of D nearest to 0 */
	y[0] = y[1] = y[2] = 0;
	k = abs(LDU[P[0]][0]) < abs(LDU[P[2]][2]) ? (abs(LDU[P[0]][0]) < abs(LDU[P[1]][1]) ? 0 : 1) : (abs(LDU[P[1]][1]) < abs(LDU[P[2]][2]) ? 1 : 2);
	y[k] = 1;
	...
	case 1:
		 ...
		 */
		y[0] = y[1] = y[2] = 0;
		k = abs(U[3 * 0 + 0]) < abs(U[3 * 0 + 2]) ? (abs(U[3 * 0 + 0]) < abs(U[3 * 0 + 1]) ? 0 : 1) : (abs(U[3 * 0 + 1]) < abs(U[3 * 0 + 2]) ? 1 : 2);
		y[k] = 1;
